...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await api.post<ApiResponse<{ user: User; token: string }>>("/auth/register", {
        name,
        email,
        password,
      });
  ...
  return (
    <div className="max-w-sm mx-auto">
      <p className="catalog-number mb-2">Entry 00</p>
      <h1 className="font-display text-3xl text-paper mb-8">Create an account</h1>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm text-muted mb-1 font-mono">Name</label>
          <input
            required
            value={name}
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-panel border hairline rounded-sm px-3 py-2 text-paper focus:border-brass outline-none"
          />
          <p className="text-muted text-xs mt-1 font-mono">Minimum 6 characters</p>
        </div>
